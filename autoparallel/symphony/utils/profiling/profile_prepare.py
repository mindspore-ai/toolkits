...
def decide_pp_dp(config, available_devices):
    ...
    if cur_multiple > 1:
        dp //= cur_multiple
    return config[dp, tp, pp, ep]

def trans_config_satisfy_rank_num(dryrun_prune_space, rank_num):
    """
    ...
    for config in dryrun_prune_space:
        trans_config = budget_profile_config_generator(config, rank_num)
...
def budget_profile_config_generator(config, available_devices):
    """
...
def profile_prepare(dryrun_prune_space, para):
    """
    prepare for profiling

    :param args: 用户输入参数
    :param dryrun_prune_space: [dp, tp, pp, ep, evaluate_peak_mem]
    :return: ordered [dp, tp, pp, ep, cost]
    """
    # 配置转换可在现有卡资源下profile的配置
    profile_configs = trans_config_satisfy_rank_num(dryrun_prune_space, para.RANK_NUM)
    profile_yaml_directory = para.OUTPUT_PATH + os.sep + 'profile_yaml' + os.sep
    generate_files(profile_configs, profile_yaml_directory, "profile", para)
