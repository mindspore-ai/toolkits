...
def train_net():
    ...
    summary_collector = ms.SummaryCollector(summary_dir="./summary_dir/summary_01", collect_freq=1,
                                            keep_default_action=False, collect_tensor_freq=2,
                                            export_options={'tensor_format':'npy'})
    if config.use_profilor:
        model.train(3, train_dataset, callbacks=[LossMonitor(), TimeMonitor(), summary_collector],
