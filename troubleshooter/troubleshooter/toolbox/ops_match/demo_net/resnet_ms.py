...
class BasicBlock(nn.Cell):
    ...
    def __init__(
        ...
        if groups != 1 or base_width != 64:
            raise ValueError(
                "BasicBlock only supports group=1 and base_width=64")
        if dilation > 1:
            raise NotImplementedError(
                "Dilation > 1 not supported in BasicBlock")
        # Both self.conv1 and self.downsample layers downsample the input when stride != 1
        self.bn10 = norm_layer(planes)
        self.conv10 = conv3x3(inplanes, planes, stride)
        self.relu = nn.ReLU()
        self.conv20 = conv3x3(planes, planes)
...
class Bottleneck(nn.Cell):
    ...
    def __init__(
        ...
        self.conv30 = conv1x1(width, planes * self.expansion)
        self.bn30 = norm_layer(planes * self.expansion)
        self.conv10 = conv1x1(inplanes, width)
        self.bn10 = norm_layer(width)
        self.conv20 = conv3x3(width, width, stride, groups, dilation)
            ...
            for _, cell in self.cells_and_names():
                if isinstance(cell, Bottleneck) and cell.bn3.gamma is not None:
                    cell.bn3.gamma.set_data(
                        "zeros", cell.bn3.gamma.shape, cell.bn3.gamma.dtype
                    )
                elif isinstance(cell, BasicBlock) and cell.bn2.weight is not None:
                    cell.bn2.gamma.set_data(
                        "zeros", cell.bn2.gamma.shape, cell.bn2.gamma.dtype
                    )
