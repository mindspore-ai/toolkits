    ...
    def __init__(self, input_size=224, n_class=1000, model_size='1.5x'):
        ...
        self.conv_last = nn.SequentialCell(
            [
                ...
                nn.BatchNorm2d(
                    num_features=self.stage_out_channels[-1], momentum=0.9),
                nn.ReLU(),
            ]
        )
        self.globalpool = nn.AvgPool2d(
            kernel_size=7, stride=7, pad_mode='valid')
        ...
        for name, m in self.cells_and_names():
            if isinstance(m, nn.Conv2d):
                if 'first' in name:
                    m.weight.set_data(
                        Tensor(
                            np.random.normal(0, 0.01, m.weight.data.shape).astype(
                                "float32"
                            )
                        )
                    )
                    ...
                    m.weight.set_data(
                        Tensor(
                            np.random.normal(
                                0, 1.0 /
                                m.weight.data.shape[1], m.weight.data.shape
                            ).astype("float32")
                        )
                    )
