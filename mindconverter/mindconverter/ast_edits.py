...
from mindconverter.code_analysis import CodeAnalyzer
from mindconverter.code_analysis import APIAnalysisSpec
from mindconverter.config import ALL_MAPPING
from mindconverter.config import NN_LIST
from mindconverter.config import ALL_TORCH_APIS
from mindconverter.config import ALL_2P_LIST
from mindconverter.config import TENSOR_DOT_LIST
from mindconverter.config import get_prompt_info
from mindconverter.common.log import logger
from mindconverter.common.exceptions import NodeTypeNotSupport
from mindconverter.forward_call import ForwardCall

LOG_FMT_INSERT = "[插入] '%s' 已经插入到被转换为文件中"
LOG_FMT_CONVERT = "[已转换] '%s' 已经被转换为了 '%s'."
LOG_FMT_CONVERT_WITH_TIPS = "[已转换] '%s' 已经被转换为了 '%s'. %s"
LOG_FMT_NOT_CONVERT = "[未转换] '%s' 没有进行转换，建议您自行进行参数转换 %s"
LOG_FMT_PROMPT_INFO = "[信息] %s"
LOG_SUGGESTION_MANUAL_CONVERT = "请注意手动转换代码以及与其关联的代码。"
...
class _ConvertReport:
    ...
    def get_logs(self):
        ...
        for log_info in self._log_body:
            log_info = "行 %d:列 %d: %s" % (log_info[1], log_info[2], log_info[3])
            if logs:
...
class AstEditVisitor(ast.NodeVisitor):
    ...
    def _convert_cell(self, cell_scope):
        ...
        logger.info("Line %3d: 开始进行转换 nn.Module %s", line_no, self._code_analyzer.get_name(cell_ast_node))
    ...
    def _modify_function_name(func_def_node, new_func_name):
        ...
        if not isinstance(func_def_node, ast.FunctionDef):
            raise NodeTypeNotSupport('【错误】该结点不是一个 ast.FunctionDef 节点类型，您可以查看对应算子的Mapping表是否存在问题。')
    ...
    def _mapping_standard_api_name(self, api_name):
        ...
        if not self._code_analyzer.is_standard_external_ref:
            # key：表示实际的ref 名称，value：标准的ref 名称
    ...
    def _infer_api_name(self, call_func_node, check_context=True):
        ...
        if standard_name in ["nn", "F", "torch", "optim"]:
    ...
    def match_api(self, call_func_node, is_forward, check_context=True):
        ...
        # print("++++++DEBUG_NAME3+++++")
        # print(standard_api_call_name)
        if standard_api_call_name in ALL_TORCH_APIS:
            match_case = ApiMatchingEnum.API_FOUND
            # print("+++++++DEBUG_NAME4+++++")
            # print(standard_api_call_name)
            # Debug is_forward
            if (not is_forward and standard_api_call_name in ALL_TORCH_APIS) or \
        ...
        if isinstance(old_node, ast.Call) and isinstance(new_node, ast.Call):
            ...
            if new_api_name == old_api_name:
                ...
                if old_parameter_num > 1:
                    msg = '参数已转换，本算子MindSpore与Pytorch存在一些差异，参考资料：'
                else:
                    if old_parameter_num == 0 and new_parameter_num == 0:
                        msg = '该API名称已经被转换为了MindSporeAPI名称'
                    else:
                        msg = '参数已转换，本算子MindSpore与Pytorch存在一些差异，参考资料：'
        return msg
    ...
    def _convert_call(self, node, matched_api_name):
        ...
        if matched_api_name in ALL_MAPPING:
            ...
            logger.warning("Line %3d: 该处找到不支持的API算子: %s%s", node.lineno, api_name, warning_info)
    ...
    def _mapping_standard_external_ref(self):
        ...
        for ref_name, ref_info in external_refs.items():
            ...
            if ref_name != 'nn' and external_ref_info.name == 'torch.nn':
                ...
                renames[ref_name] = 'F'
            elif ref_name != 'optim' and external_ref_info.name == 'torch.optim':
                renames[ref_name] = 'optim'
        return renames
