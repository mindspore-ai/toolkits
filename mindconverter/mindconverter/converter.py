...
from mindconverter.ast_edits import AstEditVisitor
    ...
    def convert(self, infile, output_dir, report_dir):
        ...
        try:
            ...
            with os.fdopen(os.open(dest_file, self.flags, self.modes), 'w') as file:
                ...
                file.write(script)
            logger.info("转换操作已完成，转换结果已被写入到 %s.", dest_file)
        except ScriptNotSupport as error:
            self._report.append('[不支持脚本] ' + error.message)
            self._report.append('[转换失败]')
            raise error
            ...
            if self._report:
                ...
                with os.fdopen(os.open(dest_report_file, self.flags, self.modes), 'a') as file:
                    file.write('\n'.join(self._report))
                logger.info("请注意查看转换报告然后进行手动调试，转换报告已被保存到 %s", dest_report_file)
