...
from mindconverter.common.log import logger
...
class MappingHelper:
    ...
    def convert(self, call_name_pt: str, args_str_pt: str):
        ...
        pt_param_input = list(args_pt.values())[0]
        pt_param_alias = list(args_pt.keys())[0]
        if("call_name" in args_pt.keys()):
            pt_call_name = args_pt["call_name"]
        ...
        elif self.ms2pt_mapping == {}:
            # print("********DEBUG********")
            # print(pt_call_name)
            # print(self.ms_api.name)
            # print(pt_param_input)
            # print(pt_param_alias)
            # print("********DEBUG********")
            if(self.ms_api.name.startswith(".")):
                expr_ms = "%s%s(%s)" % (pt_call_name, self.ms_api.name, pt_param_input)
            else:
                if(pt_param_alias == "inplace"):
                    expr_ms = "%s()" % (self.ms_api.name)
                else:
...
TORCH_OPTIM_MAPPING_PATH = os.path.realpath(os.path.join(os.path.dirname(__file__), 'mappings/torch_optim_mappings.json'))
TORCH_OPTIM_MAPPING = get_mapping_from_file(TORCH_OPTIM_MAPPING_PATH)
TORCH_OPTIM_MAPPING.update({"torch." + k : v for k, v in TORCH_OPTIM_MAPPING.items()})

ALL_MAPPING = {**NN_MAPPING, **F_MAPPING, **TORCH_DOT_MAPPING, **TENSOR_DOT_MAPPING, **TORCH_OPTIM_MAPPING}
...
TORCH_OPTIM_LIST_PATH = os.path.realpath(os.path.join(os.path.dirname(__file__), 'ops', 'torch_optim_list.json'))
TORCH_OPTIM_LIST = load_json_file(TORCH_OPTIM_LIST_PATH)
TORCH_OPTIM_LIST += ["torch." + name for name in TORCH_OPTIM_LIST]
TORCH_OPTIM_SUPPORTED = [x for x in TORCH_OPTIM_LIST if x in ALL_MAPPING]
TORCH_OPTIM_UNSUPPORTED = [x for x in TORCH_OPTIM_LIST if x not in ALL_MAPPING]

ALL_2P_LIST = F_LIST + TORCH_DOT_LIST + TENSOR_DOT_LIST + TORCH_OPTIM_LIST
ALL_TORCH_APIS = NN_LIST + F_LIST + TORCH_DOT_LIST + TENSOR_DOT_LIST + TORCH_OPTIM_LIST
ALL_SUPPORTED = NN_SUPPORTED + F_SUPPORTED + TORCH_DOT_SUPPORTED + TENSOR_DOT_SUPPORTED + TORCH_OPTIM_SUPPORTED
ALL_UNSUPPORTED = NN_UNSUPPORTED + F_UNSUPPORTED + TORCH_DOT_UNSUPPORTED + TENSOR_DOT_UNSUPPORTED + TORCH_OPTIM_UNSUPPORTED

UNSUPPORTED_WARN_INFOS_PATH = os.path.realpath(os.path.join(os.path.dirname(__file__), 'warn_info', 'unsupported_warn_infos.json'))
UNSUPPORTED_WARN_INFOS = load_json_file(UNSUPPORTED_WARN_INFOS_PATH)
...
SUPPORTED_WARN_INFOS_PATH = os.path.realpath(os.path.join(os.path.dirname(__file__), 'warn_info', 'supported_warn_infos.json'))
SUPPORTED_WARN_INFOS = load_json_file(SUPPORTED_WARN_INFOS_PATH)
