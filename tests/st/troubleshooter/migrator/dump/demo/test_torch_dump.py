...
def test_api_dump_torch_all():
    ...
    try:
        ...
        csv_list, npy_list, stack_list = get_csv_npy_stack_list(
            dump_path, 'torch')
        assert len(csv_list) == 25  # 21 apis + 4 layers
...
def test_api_dump_pt_all_with_scalar():
    ...
    try:
        ...
        csv_list, npy_list, stack_list = get_csv_npy_stack_list(
            dump_path, 'torch')
        assert len(csv_list) == 29  # 25 apis + 4 layers
        assert 'Torch_clip_0_forward_input.1' in csv_list
        assert 'Torch_clip_0_forward_input.2' in csv_list
...
def test_api_dump_torch_part():
    ...
    try:
        ...
        csv_list, npy_list, stack_list = get_csv_npy_stack_list(
            dump_path, 'torch')
        assert len(csv_list) == 7   # 6 apis + 1 layer
        assert set(csv_list) == set(npy_list)
...
def test_api_dump_torch_api_list():
    ...
    try:
        ...
        assert len(csv_list) == 10   # 9 apis + 1 layer
        assert set(csv_list) == set(npy_list)
...
def train_pt_one_step_range(data_path, dump_path, info_path=None, retain_backwad=True):
    class Net(BaseNet):
        def forward(self, x):
            api_dump_start(mode = 'range', scope = ['Torch_clip_0', 'Tensor_reshape_0'], statistic_category = ['max', 'min', 'avg', 'md5', 'l2norm'])
            x = self.conv(x)
            x = torch.clip(x, 0.2, 0.5)
            x = self.bn(x)
            x = self.relu(x)
...
def test_api_dump_torch_range():
    ...
    try:
        ...
        assert len(csv_list) == 14  # 12 apis + 2 layers
        assert set(csv_list) == set(npy_list)
